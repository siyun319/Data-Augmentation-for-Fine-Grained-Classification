# encoder with normal GAN loss + a little reconstruction loss
# why? encoder with normal GAN loss -> moda collpas
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(loader_train):
                ...
                if accuracy > 0.75:
                    if save_C:
                        if Constant.colab:
                            torch.save(net.classifier.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/C_model_accuracy_75.pt')
                        else:
                    ...
                    l_g = loss_function(labels_original, label_1.float()) + loss_function(labels_fake, label_0.float()) + loss_function(labels_re, label_0.float())
                    loss_D = torch.sum(l_g)
                    loss_D.backward()
                    optimizerD.step()
                    net.discriminator.zero_grad()
                    print("loss_D_1:",loss_D )
                    ...
                    if torch.sum(l_g) - loss_C > 5:
                      loss_G = 0.6 * torch.sum(l_g) + loss_C + 0.01 * reconstructed_loss **(1/2)
                    else:
                      loss_G = torch.sum(l_g) + loss_C + 0.01 * reconstructed_loss **(1/2)
                    #   print("2:loss_c:", loss_C)
                    # print("l_g:", l_g)
                    print("loss_G:", loss_G)
                    print("loss_C:", loss_C)
                    print("l_g:", torch.sum(l_g))
                    print("re:", 0.01 * reconstructed_loss **(1/2))
                    ...
                    loss_E = torch.sum(kl)*(1/2) + loss_function(labels_recon, label_1.float()) + 0.01 * reconstructed_loss **(1/2)
                    loss_E.backward()
                    print("reconstructed loss:", 0.01 * reconstructed_loss **(1/2))
                    print(":loss_E", loss_E)
                    optimizerE.step()
                    # lr_encoder.step()
                    net.encoder.zero_grad()
                    ...
                    with torch.no_grad():
                          ...
                          for each_label in labels:
                                if Constant.colab:
                                    if not os.path.exists("/content/gdrive/MyDrive/ic/my_model/with_labels_top_50/" + str(each_label.item())):
                                        os.makedirs("/content/gdrive/MyDrive/ic/my_model/with_labels_top_50/" + str(each_label.item()))
                                    save_image(fake_images[j].cpu().float(), '/content/gdrive/MyDrive/ic/my_model/with_labels_top_50/' + str(each_label.item()) + "/epoch{}.png".format(epoch))
                                else:
        ...
        with torch.no_grad():
                if accuracy > 0.75:
                      if Constant.colab:
                            save_image(fake_images.cpu().float(), '/content/gdrive/MyDrive/ic/my_model/with_labels_top_50_result/fake_samples_epoch_{}d.png'.format(epoch), normalize = True)
                            print("epoch end------------")
                            print("loss G:", loss_G)
                            # print("loss E:", loss_E)
                            print("l_g", l_g)
        ...
        if current_min_accuracy > 0.85:
            if Constant.colab:
                    torch.save(net.decoder.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/G_model_classification_loss.pt')
            else:
        ...
        if epoch % 100 == 0:
            if Constant.colab:
                torch.save(net.decoder.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/2_G_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(net.discriminator.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/2_D_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(net.classifier.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/2_C_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(net.encoder.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/2_E_model_classification_loss_epoch_{}.pt'.format(epoch))
                
                torch.save(optimizer_ft.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/2_optC_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(optimizerD.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/2_opt_D_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(optimizerG.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/2_opt_G_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(optimizerE.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_my_model/2_opt_E_model_classification_loss_epoch_{}.pt'.format(epoch))
            else:
...
# net.decoder.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_2/2_G_model_classification_loss_epoch_500.pt"))
# net.classifier.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_2/2_C_model_classification_loss_epoch_500.pt"))
# net.encoder.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_2/2_E_model_classification_loss_epoch_500.pt"))
# net.discriminator.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_2/2_D_model_classification_loss_epoch_500.pt"))
...
# optimizerD.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_2/2_opt_D_model_classification_loss_epoch_500.pt"))
# optimizerE.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_2/2_opt_E_model_classification_loss_epoch_500.pt"))
# optimizerG.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_2/2_opt_G_model_classification_loss_epoch_500.pt"))
# optimizer_ft.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_2/2_optC_model_classification_loss_epoch_500.pt"))
train()
