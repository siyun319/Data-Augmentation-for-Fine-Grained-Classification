# encoder with  normal GAN loss
# This file does nothing. I used this file for fine-tuning the model.
...
class DecoderBlock(nn.Module):
    def __init__(self, channel_in, channel_out, temp = 2):
        ...
        # 24 * 4
        self.conv = SpectralNorm(nn.Conv2d(channel_in, channel_out, 3, stride=1, padding=1))
...
class Decoder(nn.Module):
    def __init__(self, size = 512, z_size = latent_vector_size):
        ...
        layers.append(nn.Sequential(
            SpectralNorm(nn.Conv2d(in_channels=self.size, out_channels=3, kernel_size=3, stride=1, padding=1)),
            nn.Tanh()
        ))
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(loader_train):
                ...
                if accuracy > 0.75:
                    ...
                    l_g = loss_function(labels_original, label_1.float())
