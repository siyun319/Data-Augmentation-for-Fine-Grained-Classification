# Conditional self-attention with spectral normlization + encoder
# no good result,
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(loader_train):
                ...
                if accuracy > 0.75:
                    if save_C:
                        if Constant.colab:
                            torch.save(net.classifier.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/C_model_accuracy_75.pt')
                        else:
                    ...
                    with torch.no_grad():
                          ...
                          for each_label in labels:
                                if Constant.colab:
                                    if not os.path.exists("/content/gdrive/MyDrive/ic/encoder_4/with_labels_top_50/" + str(each_label.item())):
                                        os.makedirs("/content/gdrive/MyDrive/ic/encoder_4/with_labels_top_50/" + str(each_label.item()))
                                    save_image(fake_images[j].cpu().float(), '/content/gdrive/MyDrive/ic/encoder_4/with_labels_top_50/' + str(each_label.item()) + "/epoch{}.png".format(epoch))
                                else:
        ...
        with torch.no_grad():
                if accuracy > 0.75:
                      if Constant.colab:
                            save_image(fake_images.cpu().float(), '/content/gdrive/MyDrive/ic/encoder_4/with_labels_top_50_result/fake_samples_epoch_{}d.png'.format(epoch), normalize = True)
                            print("epoch end------------")
                            print("loss G:", loss_G)
                            # print("loss E:", loss_E)
                            print("l_g", l_g)
        ...
        if current_min_accuracy > 0.85:
            if Constant.colab:
                    torch.save(net.decoder.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/G_model_classification_loss.pt')
            else:
        ...
        if epoch % 100 == 0:
            if Constant.colab:
                torch.save(net.decoder.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/2_G_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(net.discriminator.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/2_D_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(net.classifier.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/2_C_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(net.encoder.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/2_E_model_classification_loss_epoch_{}.pt'.format(epoch))
                
                torch.save(optimizer_ft.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/2_optC_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(optimizerD.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/2_opt_D_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(optimizerG.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/2_opt_G_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(optimizerE.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_encoder_4/2_opt_E_model_classification_loss_epoch_{}.pt'.format(epoch))
            else:
