...
def calculate_mean(target_ten, input_ten, labels):
    ...
    temp = torch.zeros(target_ten.shape[0], target_ten.shape[1], target_ten.shape[2], target_ten.shape[3])
    for i, label in enumerate(labels):
        counts[label] = counts[label] + 1
        temp[label] += input_ten[i]

    for label in labels:
        temp[label] = torch.div(target_ten[label], counts[label])

  # Update using average mean
    target_ten = torch.mul(target_ten, 0.2) + torch.mul(temp, 0.8)
    return target_ten
...
def calculate_difference(target, input, labels):
  temp = 0
  for i, label in enumerate(labels):
    temp = temp + (target[label] - input[i])**2
  temp = 0.5 * temp.sum()
  return temp
...
def train():
    ...
    for epoch in range(301, num_epochs):
        net.train()
        running_corrects = 0
        corrects = 0
        ...
        historical_c_feature = torch.zeros(n_classes, 1024, 7, 1)
        historical_d_feature = torch.zeros(n_classes, 128, 5, 1)
        for i, data in enumerate(loader_train):
                ...
                if accuracy > 0.75:
                    if save_C:
                        if Constant.colab:
                            torch.save(net.classifier.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/C_model_accuracy_75.pt')
                        else:
                    ...
                    l_g = loss_function(labels_original, label_1.float()) + loss_function(labels_fake, label_0.float()) + loss_function(labels_recon, label_0.float())
                    ...
                    correct_discriminations = (preds_real == labels)
                    ...
                    historical_c_feature = calculate_mean(historical_c_feature, feature_c_real.detach(), labels)
                    historical_d_feature = calculate_mean(historical_d_feature, feature_d_real.detach(), labels)
                    ...
                    target = historical_c_feature.to(device)
                    target_d = historical_d_feature.to(device)

                    
                    temp = reconstructed_loss +  0.5 * ((feature_c_real.detach() - feature_c_re)**2).sum() + 0.5 * ((feature_d_real.detach() - feature_d_recon) ** 2).sum()
                    print("target_d:", target_d.shape)
                    # temp__ = calculate_difference(target_d - feature_d_fake, labels)
                    # temp______ = 0.5 * ((target - feature_c_fake)**2).sum()
                    temp2 = calculate_difference(target.detach(), feature_c_fake, labels)+ calculate_difference(target_d.detach(), feature_d_fake, labels)
                    del target
                    # print(reconstructed_loss)

                    loss_G = temp + 0.001 * temp2
                    ...
                    temp = reconstructed_loss +  0.5 * ((feature_c_real - feature_c_re)**2).sum() + 0.5 * ((feature_d_real - feature_d_recon) ** 2).sum()
                    loss_E = 3 * torch.sum(kl) + temp
                    loss_E.backward()
                    print(":loss_E", loss_E)
                    optimizerE.step()
                    # lr_encoder.step()
                    ...
                    with torch.no_grad():
                          ...
                          for each_label in labels:
                                if Constant.colab:
                                    if not os.path.exists("/content/gdrive/MyDrive/ic/CVAEGAN/with_labels_top_50/" + str(each_label.item())):
                                        os.makedirs("/content/gdrive/MyDrive/ic/CVAEGAN/with_labels_top_50/" + str(each_label.item()))
                                    save_image(fake_images[j].cpu().float(), '/content/gdrive/MyDrive/ic/CVAEGAN/with_labels_top_50/' + str(each_label.item()) + "/epoch{}.png".format(epoch))
                                else:
        ...
        with torch.no_grad():
                if accuracy > 0.75:
                      if Constant.colab:
                            save_image(fake_images.cpu().float(), '/content/gdrive/MyDrive/ic/CVAEGAN/with_labels_top_50_result/fake_samples_epoch_{}d.png'.format(epoch), normalize = True)
                            print("epoch end------------")
                            print("loss G:", loss_G)
                            # print("loss E:", loss_E)
                            print("l_g", l_g)
                            ...
                            print("loss D:", loss_D)
                            print("loss C:", loss_C)
                            print("current_Accuracy_epoch:", sum(current_accuracy_list)/float(len(current_accuracy_list)))
                      else:
        ...
        if current_min_accuracy > 0.85:
            if Constant.colab:
                    torch.save(net.decoder.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/G_model_classification_loss.pt')
            else:
        ...
        if epoch % 100 == 0:
            if Constant.colab:
                torch.save(net.decoder.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_G_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(net.discriminator.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_D_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(net.classifier.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_C_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(net.encoder.state_dict(), '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_E_model_classification_loss_epoch_{}.pt'.format(epoch))
                
                torch.save(optimizer_ft.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_optC_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(optimizerD.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_opt_D_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(optimizerG.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_opt_G_model_classification_loss_epoch_{}.pt'.format(epoch))
                torch.save(optimizerE.state_dict(),  '/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_opt_E_model_classification_loss_epoch_{}.pt'.format(epoch))
            else:
...
net.encoder.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_E_model_classification_loss_epoch_300.pt"))
net.discriminator.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_D_model_classification_loss_epoch_300.pt"))
net.classifier.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_C_model_classification_loss_epoch_300.pt"))
net.decoder.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_G_model_classification_loss_epoch_300.pt"))
...
optimizerD.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_opt_D_model_classification_loss_epoch_300.pt"))
optimizerG.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_opt_G_model_classification_loss_epoch_300.pt"))
optimizerD.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_opt_D_model_classification_loss_epoch_300.pt"))
optimizerE.load_state_dict(torch.load("/content/gdrive/MyDrive/ic/trained_models/top_50_CVAEGAN/2_opt_E_model_classification_loss_epoch_300.pt"))
